...
// Set width for inputs in horizontal search bar -----------------------------------------------------------------------
    ...
    $( "#redefine-search-form" ).load( "/static/base_template_user/assets/external/_search-bar.html", function() {
        setInputsWidth();
        //autoComplete();
    });
...
function autoComplete(){
    if( !$("script[src='/static/base_template_user/assets/js/leaflet.js']").length ){
...
function drawOwlCarousel(_rtl){
    $.getScript( "/static/base_template_user/assets/js/owl.carousel.min.js", function( data, textStatus, jqxhr ) {
        ...
        if( category == 'real_estate' ){
            if( json.data[a].item_specific ){
                if( json.data[a].item_specific.bedrooms ){
                    itemSpecific += '<span title="Bedrooms"><img src="/static/base_template_user/assets/img/bedrooms.png">' + json.data[a].item_specific.bedrooms + '</span>';
                }
                if( json.data[a].item_specific.bathrooms ){
                    itemSpecific += '<span title="Bathrooms"><img src="/static/base_template_user/assets/img/bathrooms.png">' + json.data[a].item_specific.bathrooms + '</span>';
                }
                if( json.data[a].item_specific.area ){
                    itemSpecific += '<span title="Area"><img src="/static/base_template_user/assets/img/area.png">' + json.data[a].item_specific.area + '<sup>2</sup></span>';
                }
                if( json.data[a].item_specific.garages ){
                    itemSpecific += '<span title="Garages"><img src="/static/base_template_user/assets/img/garages.png">' + json.data[a].item_specific.garages + '</span>';
                }
                return itemSpecific;
            }
        }
...
function quickView(id){
    $.ajax({
        type: 'POST',
        url: '/static/base_template_user/assets/external/_modal.html',
        data: id,
