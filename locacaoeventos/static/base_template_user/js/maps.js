   ...
   $.get("/static/base_template_user/assets/external/_infobox.js", function() {
    ...
    function gMap(){
        ...
        for (var i = 0; i < json.data.length; i++) {
            ...
            if( json.data[i].type_icon ) var type_icon = '<img src="' + json.data[i].type_icon +  '">';
            else type_icon = '';

            if( json.data[i].featured ) var featured = 'featured ';
            else type_icon = ' ';
            ...
            google.maps.event.addListener(marker, 'click', (function(marker, i) {
                ...
                return function() {
                    $.ajax({
                        type: 'POST',
                        url: '/static/base_template_user/assets/external/_infobox.html',
                        data: json.data[i].id,
        ...
        google.maps.event.addListener(map, 'idle', function() {
            ...
            for( var a = 0; a < json.data.length; a++ ){
                if( map.getBounds().contains( new google.maps.LatLng( json.data[a].latitude, json.data[a].longitude ) ) ) {
                    if ( tempVar < 20 ) {
                        tempVar++;
                        pushItemsToArray(json, json.data[a].id, visibleItemsArray, 'resultsSidebar');
                    }
                }
            }
...
function pushItemsToArray(json, id, visibleItemsArray, method){
    $.ajax({
        type: 'POST',
        url: '/static/base_template_user/assets/external/_result.html',
        data: id,
        success: function (data) {
...
function multiChoice(sameLatitude, sameLongitude, json) {
    ...
    $.each(json.data, function(a) {
        if( json.data[a].latitude == sameLatitude && json.data[a].longitude == sameLongitude ) {
            pushItemsToArray(json, a, multipleItems);
        }
    });

    $('body').append('<div class="modal-window multichoice fade_in"></div>');

    $(multipleItems).promise().done(function() {
...
function animateOSMMarkers(map, loadedMarkers, json){
    ...
    $.each( loadedMarkers, function (i) {
        if ( bounds.contains( loadedMarkers[i].getLatLng() ) ) {
            var category = json.data[i].category;
            pushItemsToArray(json, i, visibleItemsArray);
